...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('contrato_producto', function (Blueprint $table) {
            $table->uuid('uuid_temp')->nullable()->after('id');
        });

        // 2. Poblar con UUIDs (ahora la columna YA existe)
        DB::table('contrato_producto')->orderBy('id')->chunk(1000, function ($personas) {
            foreach ($personas as $persona) {
                DB::table('contrato_producto')
                    ->where('id', $persona->id)
        ...
        DB::statement('ALTER TABLE contrato_producto MODIFY id INTEGER NOT NULL');
